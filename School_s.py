...
class School:
    def __init__(self, window):
        ...
        labels = [
            ...
            "Principal Name:", "Joining Date:", "School Email ID:"
        ]
        entry_list = []
        ...
        for i, label_text in enumerate(right_labels):
            ...
            if label_text == "Magazine Copy:":
                # magazine_copy = tk.Entry(input_frame, bg="white", font=("Helvetica", 20), width=10,
                #                          state="readonly", textvariable=tk.StringVar())
                # magazine_copy.bind("<Button-1>", lambda event: open_text_file())
                magazine_copy = tk.Button(input_frame, text="", font=("Helvetica bold", 16), width=10, command=lambda event : open_text_file())
